...
async function generatePDF(resumeData, filePath) {
  return new Promise((resolve, reject) => {
    ...
    // Header
    doc.fontSize(24).fillColor('#4f46e5').text(resumeData.name, { align: 'center' });
    doc.fontSize(18).fillColor('#4b5563').text('Blockchain Developer', { align: 'center' });
    doc.moveDown(1);
    ...
    doc.fontSize(14).fillColor('#4f46e5').text('Contact', doc.page.margins.left, yPosition);
    yPosition += 20;
    doc.fontSize(12).fillColor('#4b5563');
    doc.text(`Phone: ${resumeData.contact.phone}`, doc.page.margins.left, yPosition);
    yPosition += 15;
    doc.text(`Email: ${resumeData.contact.email}`, doc.page.margins.left, yPosition);
    yPosition += 15;
    doc.text(`LinkedIn: ${resumeData.contact.linkedin}`, doc.page.margins.left, yPosition);
    yPosition += 15;
    doc.text(`GitHub: ${resumeData.contact.github}`, doc.page.margins.left, yPosition);
    yPosition += 15;
    doc.text(`Portfolio: ${resumeData.contact.portfolio}`, doc.page.margins.left, yPosition);
    yPosition += 20;

    doc.fontSize(14).fillColor('#4f46e5').text('Skills', doc.page.margins.left, yPosition);
    yPosition += 20;
    doc.fontSize(12).fillColor('#4b5563');
    resumeData.skills.forEach(skill => {
      doc.text(`• ${skill}`, doc.page.margins.left, yPosition);
      yPosition += 15;
    });
    yPosition += 10;

    doc.fontSize(14).fillColor('#4f46e5').text('Languages', doc.page.margins.left, yPosition);
    yPosition += 20;
    doc.fontSize(12).fillColor('#4b5563');
    doc.text(`• ${resumeData.languages}`, doc.page.margins.left, yPosition);
    yPosition += 20;

    // Right Column: Profile, Experience, Education, Projects, Certifications, Achievements & Awards
    let rightYPosition = doc.page.margins.top + 60; // Align with left column
    doc.fontSize(14).fillColor('#4f46e5').text('Profile', doc.page.margins.left + columnWidth + 20, rightYPosition);
    rightYPosition += 20;
    doc.fontSize(12).fillColor('#4b5563').text(resumeData.profile, doc.page.margins.left + columnWidth + 20, rightYPosition, { width: columnWidth * 2 });
    rightYPosition += doc.heightOfString(resumeData.profile, { width: columnWidth * 2 }) + 20;

    doc.fontSize(14).fillColor('#4f46e5').text('Experience', doc.page.margins.left + columnWidth + 20, rightYPosition);
    rightYPosition += 20;
    resumeData.experience.forEach(exp => {
      doc.fontSize(12).fillColor('#4b5563').text(exp.title, doc.page.margins.left + columnWidth + 20, rightYPosition, { width: columnWidth * 2 });
      rightYPosition += 15;
      exp.details.forEach(detail => {
        doc.text(`• ${detail}`, doc.page.margins.left + columnWidth + 25, rightYPosition, { width: columnWidth * 2 - 5 });
        rightYPosition += doc.heightOfString(detail, { width: columnWidth * 2 - 5 }) + 5;
      });
      rightYPosition += 10;
    });

    doc.fontSize(14).fillColor('#4f46e5').text('Education', doc.page.margins.left + columnWidth + 20, rightYPosition);
    rightYPosition += 20;
    doc.fontSize(12).fillColor('#4b5563').text(resumeData.education, doc.page.margins.left + columnWidth + 20, rightYPosition, { width: columnWidth * 2 });
    rightYPosition += doc.heightOfString(resumeData.education, { width: columnWidth * 2 }) + 20;

    doc.fontSize(14).fillColor('#4f46e5').text('Projects', doc.page.margins.left + columnWidth + 20, rightYPosition);
    rightYPosition += 20;
    resumeData.projects.forEach(project => {
      doc.fontSize(12).fillColor('#4b5563').text(`${project.name} (${project.techStack})`, doc.page.margins.left + columnWidth + 20, rightYPosition, { width: columnWidth * 2 });
      rightYPosition += 15;
      doc.text(`Link: ${project.liveLink}`, doc.page.margins.left + columnWidth + 25, rightYPosition, { width: columnWidth * 2 - 5 });
      rightYPosition += 15;
      doc.text(project.summary, doc.page.margins.left + columnWidth + 25, rightYPosition, { width: columnWidth * 2 - 5 });
      rightYPosition += doc.heightOfString(project.summary, { width: columnWidth * 2 - 5 }) + 10;
    });

    doc.fontSize(14).fillColor('#4f46e5').text('Certifications', doc.page.margins.left + columnWidth + 20, rightYPosition);
    rightYPosition += 20;
    doc.fontSize(12).fillColor('#4b5563').text(resumeData.certifications, doc.page.margins.left + columnWidth + 20, rightYPosition, { width: columnWidth * 2 });
    rightYPosition += doc.heightOfString(resumeData.certifications, { width: columnWidth * 2 }) + 20;

    doc.fontSize(14).fillColor('#4f46e5').text('Achievements & Awards', doc.page.margins.left + columnWidth + 20, rightYPosition);
    rightYPosition += 20;
    doc.fontSize(12).fillColor('#4b5563');
    doc.text(`• ${resumeData.achievements}`, doc.page.margins.left + columnWidth + 20, rightYPosition, { width: columnWidth * 2 });
    rightYPosition += 15;
    doc.text(`• ${resumeData.awards}`, doc.page.margins.left + columnWidth + 20, rightYPosition, { width: columnWidth * 2 });
